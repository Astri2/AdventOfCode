...
for iteration in range(max_iteration):
    ...
    if(robots in hist): 
        id = hist.index(robots)
        print("cycle!", id, iteration)
        correct_iteration = max_iteration % iteration
        compute_res(hist[correct_iteration])
        exit(0)
    hist.append(deepcopy(robots))
compute_res(robots)